    ...
    if (contentLen) {
        ...
        if (searchJsonResponse("ok:")) {
            if (strcmp(keyValue, "true")) {
                ...
                if (searchJsonResponse("description:"))
                    LOG_ERR("Telegram error: %s", keyValue);
                else
                    LOG_ERR("Telegram error, but description not retrieved");
...
static bool sendTgramHeader(const char* tmethod, const char* contentType, const char* dataType, size_t fileSize, const char* fileName, const char* caption) {
    if (connectTelegram()) {
        ...
        if (strlen(tgramToken)) {
            ...
            if (res) {
                ...
                if (searchJsonResponse("username:")) {
                    ...
                    LOG_ERR("getMe response not parsed %s", tgramBuff);
            } else
                LOG_ERR("Failed to communicate with Telegram server");
        } else
            LOG_WRN("No Telegram Bot token supplied");
    } else
        LOG_INF("Telegram not being used");
    return false;
}
                ...
                if (lastUpdate < update_id) {
                    ...
                    if (searchJsonResponse("chat:{id:")) {
                        if (!strcmp(tgramChatId, keyValue)) {
                            ...
                            LOG_WRN("Message from unknown chat id: %s", keyValue);
                    } else
                        LOG_ERR("No chat id found");
                } else
                    LOG_WRN("Old update_id: %d", update_id);
            }  // no update_id, ignore
        }
    } else {
    ...
    if (connectTelegram()) {
        ...
        if (df) {
            if (df.size() < MAX_TGRAM_SIZE) {
                ...
                snprintf(errMsg, sizeof(errMsg) - 1, "File size too large: %s", fmtSize(df.size()));
        } else
            snprintf(errMsg, sizeof(errMsg) - 1, "File does not exist or cannot be opened: %s", fileName);
        if (strlen(errMsg)) {
            LOG_ERR("%s", errMsg);
