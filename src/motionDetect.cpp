...
static bool tinyMLclassify() {
    ...
    if (res == EI_IMPULSE_OK) {
        ...
        LOG_ERR("Failed to run classifier (%d)", res);
    return out;
}
#endif
...
bool checkMotion(camera_fb_t* fb, bool motionStatus) {
    ...
    for (int i = 0; i < resizeDimLen; i += colorDepth) {
        ...
        if (dbgMotion)
            for (int j = 0; j < RGB888_BYTES; j++) changeMap[(i * stride) + j] = currPix;  // grayscale
        ...
        if (abs((int)currPix - (int)prevPix) > detectChangeThreshold) {
            if (i > startPixel && i < endPixel)
                ...
                pixVal = 80;  // show inactive changed pixel as dark red color in changeMap image
            if (dbgMotion) {
    ...
    if (changeCount > moveThreshold) {
        ...
        if (!motionStatus && motionCnt >= detectMotionFrames) {
            ...
            if (!dbgMotion && mlUse)
                if (!tinyMLclassify()) motionCnt = 0;  // not classified, so cancel motion
#endif
            if (motionCnt) {
#if INCLUDE_SMTP
                if (smtpUse) {
