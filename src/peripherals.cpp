...
void prepTemperature() {
    ...
    if (ds18b20Pin < EXTPIN) {
        if (ds18b20Pin) {
            ...
            LOG_WRN("No DS18B20 pin defined, using chip sensor if present");
    }
#endif
...
static void setupBatt() {
    if (voltUse && (voltPin < EXTPIN)) {
        if (voltPin) {
            ...
            LOG_WRN("No voltage pin defined");
    }
}
...
static void setupLamp() {
    ...
    if ((lampPin < EXTPIN) && lampUse) {
        if (lampPin) {
            ...
            if (rmtWS2812 == NULL)
                LOG_ERR("Failed to setup WS2812 with pin %u", lampPin);
            else {
...
void prepMotors() {
    if (RCactive) {
        if (motorFwdPin > 0) {
            ...
            LOG_WRN("RC motor pins not defined");
    }
}
...
void motorSpeed(int speedVal) {
    ...
    if (speedVal == 0)
        motorStop();
    else if (motorFwdPin > 0 && speedVal > 0.0)
        motorForward(speedValFloat);
    else if (motorRevPin > 0 && speedVal < 0.0)
        motorReverse(speedValFloat);
}
