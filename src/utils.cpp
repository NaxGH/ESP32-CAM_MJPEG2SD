...
static const char* wifiStatusStr(wl_status_t wlStat) {
    switch (wlStat) {
    case WL_NO_SHIELD:
        return "wifi not initialised";
    case WL_IDLE_STATUS:
        return "WL_IDLE_STATUS";
    case WL_NO_SSID_AVAIL:
        return "not available, use AP";
...
static void onWiFiEvent(WiFiEvent_t event) {
    ...
    case ARDUINO_EVENT_WIFI_STA_START:
        LOG_INF("Wifi Station started, connecting to: %s", ST_SSID);
        break;
    case ARDUINO_EVENT_WIFI_STA_STOP:
        LOG_INF("Wifi Station stopped %s", ST_SSID);
        break;
    ...
    case ARDUINO_EVENT_WIFI_STA_GOT_IP:
        LOG_INF("Wifi Station IP, use '%s://%s' to connect", useHttps ? "https" : "http", WiFi.localIP().toString().c_str());
        break;
    ...
    case ARDUINO_EVENT_WIFI_STA_DISCONNECTED:
        LOG_INF("WiFi Station disconnected");
        break;
    case ARDUINO_EVENT_WIFI_AP_STACONNECTED:
        LOG_INF("WiFi AP client connection");
        break;
    case ARDUINO_EVENT_WIFI_AP_STADISCONNECTED:
        LOG_INF("WiFi AP client disconnection");
        break;
    default:
        LOG_WRN("WiFi Unhandled event %d", event);
        break;
    }
}
...
static void pingSuccess(esp_ping_handle_t hdl, void* args) {
    ...
    if (DEBUG_MEM) {
        ...
        if (freeStack < minStack) {
            minStack = freeStack;
            if (freeStack < MIN_STACK_FREE)
                LOG_WRN("Task ping stack space only: %u", freeStack);
            else
                LOG_INF("Task ping stack space reduced to: %u", freeStack);
        }
    }
    resetWatchDog();
    statusCheck();
        ...
        if (remoteServerConnect(hclient, EXT_IP_HOST, HTTPS_PORT, "")) {
            ...
            if (https.begin(hclient, EXT_IP_HOST, HTTPS_PORT, "/", true)) {
                ...
                if (httpCode == HTTP_CODE_OK) {
                    ...
                    LOG_ERR("Request failed, error: %s", https.errorToString(httpCode).c_str());
...
bool remoteServerConnect(WiFiClientSecure& sclient, const char* serverName, uint16_t serverPort, const char* serverCert) {
    ...
    if (sclient.connected())
        ...
        if (ESP.getFreeHeap() > TLS_HEAP) {
            ...
            if (useSecure && strlen(serverCert))
                sclient.setCACert(serverCert);
            else
                sclient.setInsecure();  // no cert check
            ...
            LOG_WRN("Insufficient heap %s for %s TLS session", fmtSize(ESP.getFreeHeap()), serverName);
    }
    return false;
}
...
void formatElapsedTime(char* timeStr, uint32_t timeVal, bool noDays) {
    ...
    if (noDays)
        sprintf(timeStr, "%02u:%02u:%02u", hours, mins, secs);
    else
...
uint32_t checkStackUse(TaskHandle_t thisTask, int taskIdx) {
    ...
    if (thisTask != NULL) {
        ...
        if (freeStack < minStack[taskIdx]) {
            ...
            if (freeStack < MIN_STACK_FREE)
                LOG_WRN("Task %s stack space only: %u", pcTaskGetTaskName(thisTask), freeStack);
            else
                LOG_INF("Task %s stack space reduced to %u", pcTaskGetTaskName(thisTask), freeStack);
        }
    }
    return freeStack;
}
...
static void ramLogStore(size_t msgLen) {
    ...
    if (mlogEnd + msgLen >= RAM_LOG_LEN) {
        ...
        memcpy(messageLog + mlogEnd, outBuf, msgLen);
    mlogEnd += msgLen;
}
...
void logPrint(const char* format, ...) {
    ...
    if (xSemaphoreTake(logMutex, pdMS_TO_TICKS(logWait)) == pdTRUE) {
        ...
        if (monitorOpen)
            ...
            delay(10);  // allow time for other tasks
        if (sdLog) {
            if (log_remote_fp != NULL) {
...
void partitionInfo() {
    // You can also iterate over all partitions if needed
    esp_partition_iterator_t iterator = esp_partition_find(ESP_PARTITION_TYPE_APP, ESP_PARTITION_SUBTYPE_ANY, NULL);
    while (iterator != NULL) {
        const esp_partition_t* partition = esp_partition_get(iterator);
        LOG_INF("Label: %s | Type: %d | Subtype: %d | Size: %d bytes | Address: 0x%08x", 
                partition->label, partition->type, partition->subtype, partition->size, partition->address);
...
static esp_sleep_wakeup_cause_t printWakeupReason() {
    ...
    case ESP_SLEEP_WAKEUP_EXT0:
        LOG_INF("Wakeup by external signal using RTC_IO");
        break;
    case ESP_SLEEP_WAKEUP_EXT1:
        LOG_INF("Wakeup by external signal using RTC_CNTL");
        break;
    case ESP_SLEEP_WAKEUP_TIMER:
        LOG_INF("Wakeup by internal timer");
        break;
    case ESP_SLEEP_WAKEUP_TOUCHPAD:
        LOG_INF("Wakeup by touchpad");
        break;
    case ESP_SLEEP_WAKEUP_ULP:
        LOG_INF("Wakeup by ULP program");
        break;
    case ESP_SLEEP_WAKEUP_GPIO:
        LOG_INF("Wakeup by GPIO");
        break;
    case ESP_SLEEP_WAKEUP_UART:
        LOG_INF("Wakeup by UART");
        break;
    default:
        LOG_INF("Wakeup by reset");
        break;
    }
...
static esp_reset_reason_t printResetReason() {
    ...
    case ESP_RST_UNKNOWN:
        LOG_INF("Reset for unknown reason");
        break;
    case ESP_RST_POWERON:
        LOG_INF("Power on reset");
        break;
    case ESP_RST_EXT:
        LOG_INF("Reset from external pin");
        break;
    case ESP_RST_SW:
        LOG_INF("Software reset via esp_restart");
        break;
    case ESP_RST_PANIC:
        LOG_INF("Software reset due to exception/panic");
        break;
    case ESP_RST_INT_WDT:
        LOG_INF("Reset due to interrupt watchdog");
        break;
    case ESP_RST_TASK_WDT:
        LOG_INF("Reset due to task watchdog");
        break;
    case ESP_RST_WDT:
        LOG_INF("Reset due to other watchdogs");
        break;
    case ESP_RST_DEEPSLEEP:
        LOG_INF("Reset after exiting deep sleep mode");
        break;
    case ESP_RST_BROWNOUT:
        LOG_INF("Brownout reset (software or hardware)");
        break;
    case ESP_RST_SDIO:
        LOG_INF("Reset over SDIO");
        break;
    default:
        LOG_WRN("Unhandled reset reason");
        break;
    }
    return bootReason;
}
