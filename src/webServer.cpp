...
esp_err_t extractHeaderVal(httpd_req_t* req, const char* variable, char* value) {
    ...
    if (!hdrFieldLen)
        LOG_WRN("Field %s not present", variable);
    else if (hdrFieldLen >= IN_FILE_NAME_LEN - 1)
        LOG_WRN("Field %s value too long (%d)", variable, hdrFieldLen);
    else {
...
static esp_err_t webHandler(httpd_req_t* req) {
    ...
    if (!strcmp(variable, "OTA.htm")) {
        ...
        LOG_WRN("Unknown file type %s", variable);
...
static esp_err_t uploadHandler(httpd_req_t* req) {
    ...
    if (res == ESP_OK) {
        ...
        if (strstr(inFileName, ".bin") != NULL) {
            ...
            if (Update.hasError())
                LOG_ERR("OTA failed with error: %s", Update.errorString());
            else
            ...
            if (!uf)
                LOG_ERR("Failed to open %s on storage", inFileName);
            else {
                // obtain file content
                do {
                    bytesRead = httpd_req_recv(req, jsonBuff, rxSize);
                ...
                if (res == ESP_OK)
                    LOG_INF("Uploaded file %s", inFileName);
                else
                    LOG_ERR("Failed to upload file %s", inFileName);
            }
        }
    }
    return res;
