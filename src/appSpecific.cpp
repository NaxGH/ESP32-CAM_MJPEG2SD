...
bool updateAppStatus(const char* variable, const char* value) {
    ...
    else if (!strcmp(variable, "nvrStream"))
        nvrStream = (bool)intVal;
    else if (!strcmp(variable, "lswitch"))
        nightSwitch = intVal;
#if INCLUDE_FTP_HFS
    else if (!strcmp(variable, "upload"))
        fsFileOrFolder(value);
#endif
    ...
    // peripherals
    else if (!strcmp(variable, "useIOextender"))
        useIOextender = (bool)intVal;
        ...
        if (!lampType)
            setLamp(lampLevel);  // manual
        else
            setLamp(0);
    } else if (!strcmp(variable, "servoUse"))
        servoUse = (bool)intVal;
        ...
        voltInterval = intVal;
    else if (!strcmp(variable, "camPan"))
        setCamPan(intVal);
        ...
        pwmFreq = intVal;
    else if (!strcmp(variable, "RClights"))
        setLights((bool)intVal);
    ...
    else if (!strcmp(variable, "camHub"))
        camHub = (bool)intVal;
...
void tgramAlert(const char* subject, const char* message) {
    ...
    if (pos1 != NULL && pos2 != NULL) {
        ...
        LOG_ERR("Unable to send motion alert");
}
