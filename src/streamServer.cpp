...
static void sustainTask(void* p) {
    ...
    while (true) {
        ...
        else if (!strcmp(sustainReq[i].activity, "playback"))
            showPlayback(sustainReq[i].req);
        else if (!strcmp(sustainReq[i].activity, "stream"))
            showStream(sustainReq[i].req, i);
        else {
...
esp_err_t appSpecificSustainHandler(httpd_req_t* req) {
    ...
    if (extractQueryKeyVal(req, variable, value) == ESP_OK) {
        ...
        if (taskNum < numStreams) {
            ...
            httpd_resp_set_status(req, "400 Invalid task num");
    } else
        httpd_resp_set_status(req, "400 Bad URL");
