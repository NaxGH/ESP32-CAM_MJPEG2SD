...
static void getSensorData() {
    ...
    if (bmp280.hasValue()) {
        ...
        for (int i = 0; i < 2; i++) highPoint[i] += sprintf(teleBuf[i] + highPoint[i], "-,-,-,");
        ...
        while (capturing) {
            ...
            for (int i = 0; i < NUM_BUFF; i++) {
                if (highPoint[i] >= RAMSIZE) {
                    highPoint[i] -= RAMSIZE;
                    if (i)
                        srtFile.write((uint8_t*)teleBuf[i], RAMSIZE);
                    else
                        teleFile.write((uint8_t*)teleBuf[i], RAMSIZE);
...
void prepTelemetry() {
    ...
    if (teleUse) {
        ...
        if (setupSensors())
            xTaskCreate(&telemetryTask, "telemetryTask", TELEM_STACK_SIZE, NULL, 3, &telemetryHandle);
        else
            teleUse = false;
