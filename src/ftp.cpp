...
static void postHeader(const char* tmethod, const char* contentType, bool isFile, size_t fileSize, const char* fileName) {
...
static bool ftpConnect() {
    ...
    if (rclient.connect(fsServer, fsPort)) {
        LOG_DBG("FTP connected at %s:%u", fsServer, fsPort);
    } else {
...
static bool ftpStoreFile(File& fh) {
    ...
    if (res) {
        ...
        LOG_ERR("File transfer not successful");
    return res;
}
...
bool fsFileOrFolder(const char* fileFolder) {
    ...
    if (!uploadInProgress) {
        ...
        LOG_WRN("Unable to upload %s as another upload in progress", storedPathName);
    return false;
}
